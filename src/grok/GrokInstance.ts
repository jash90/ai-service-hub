...
export default class GrokInstance {
    ...
    async chat(prompt: string, systemPrompt: string | null = null, model: ModelGrok = ModelGrok.grok1, format: ResponseFormat = { type: "text" }): Promise<string | null> {
        try {
            const messages = [
    ...
    async vision(prompt: string, base64Image: string, systemPrompt: string, model: string = ModelGrok.grok1Vision): Promise<string | null | undefined> {
        try {
            const messages = [
                {
                    role: "user",
